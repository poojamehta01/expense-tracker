...
While amount editing is active, month/person filters, copying, and other tabs are locked until the edit is saved or cancelled, preventing silent loss of unsaved values. Choosing **Map** during amount editing explains that the pending amount changes must be saved or cancelled first. A Budget write refreshes only the Dashboard budget card for the Dashboard's own selected month; it does not replace Dashboard transactions or KPIs with another month.
